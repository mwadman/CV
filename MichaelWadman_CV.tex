...
\lfoot{\fontseries{l}\selectfont \today}
\cfoot{\fontseries{l}\selectfont \name{} {\scriptsize \\ Written in \LaTeX}}
\rfoot{\fontseries{l}\selectfont \thepage}
...
\usepackage[bookmarks=false]{hyperref} % Disables bookmarks when opened in PDF viewer. Needs to be set in \usepackage for some reason
    ...
    pdftitle={Michael Wadman - CV}, % As shown when opened in PDF viewer % chktex 8
}
...
% Variables
\newcommand{\name}{Michael Wadman}
\newcommand{\position}{Solutions Architect/Principal Engineer}
...
\newcommand{\bulletedlist}[1]{%
    \fontseries{l}\selectfont \small \raggedright{}
    \begin{itemize}
    \itemsep0em
    #1
    \end{itemize}
...
\newcommand{\positionanddates}[2]{%
    \fontseries{l}\selectfont \textsc{#1} \hfill {\fontseries{l}\selectfont \small \textit{#2}}
}
\newcommand{\yearachievementdescription}[4]{%
    #1 \hspace{10mm} \textbf{#2} \hspace{0.2mm} #3 \textit{#4}
}
\newcommand{\expiredachievement}[2]{%
    \hspace{18mm} \textbf{#1} \hspace{0.2mm} #2
}
